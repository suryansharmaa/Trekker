...
const StatCard = ({ title, value, color }) => (
  <div className="bg-[#313335] p-6 rounded-lg border border-[#4e5254] flex flex-col justify-between">
    <h3 className="text-[#A9B7C6] text-sm uppercase tracking-wider mb-2 font-mono">{title}</h3>
  ...
  return (
    <div className="bg-[#313335] rounded-lg border border-[#4e5254] overflow-hidden">
      <div className="bg-[#3C3F41] px-4 py-2 border-b border-[#242627] flex items-center justify-between">
         <span className={`font-mono text-sm tracking-wide ${themeColor}`}>{platform}.java</span>
      </div>
      <div className="p-4 space-y-4">
         ...
         {data.rating > 0 && (
             <div className="pt-2 border-t border-[#4e5254] flex justify-between items-center">
                 <span className="text-[#808080] text-xs font-mono uppercase">Rating</span>
...
const DashboardContainer = () => {
    ...
    const fetchStats = async () => {
      ...
      setError('');
      try {
        const response = await axios.post('http://127.0.0.1:5000/api/stats', usernames);
        ...
        if (err.response && err.response.data && err.response.data.error) {
            ...
            msg = 'Network Error: Cannot reach http://localhost:5000. Ensure the backend server is running.';
        }
    ...
    if (loading) {
      return (
        <div className="flex-grow flex flex-col items-center justify-center space-y-4">
          ...
          <p className="text-[#A9B7C6] font-mono animate-pulse">Compiling User Data...</p>
        </div>
      );
    }
    ...
    if (error) {
       return (
         <div className="bg-[#482828] border border-[#ff5252] text-[#ff5252] p-4 rounded font-mono mt-8">
            <p>Error: {error}</p>
            <button onClick={() => navigate('/')} className="mt-4 text-white underline hover:text-gray-300">Return to Configuration</button>
         </div>
       );
    }
    ...
    // Aggregate totals
    const totalSolved = (stats.leetcode?.totalSolved || 0) + (stats.codeforces?.totalSolved || 0) + (stats.geeksforgeeks?.totalSolved || 0);
    const totalEasy = (stats.leetcode?.easySolved || 0) + (stats.codeforces?.easySolved || 0) + (stats.geeksforgeeks?.easySolved || 0);
    const totalMedium = (stats.leetcode?.mediumSolved || 0) + (stats.codeforces?.mediumSolved || 0) + (stats.geeksforgeeks?.mediumSolved || 0);
    const totalHard = (stats.leetcode?.hardSolved || 0) + (stats.codeforces?.hardSolved || 0) + (stats.geeksforgeeks?.hardSolved || 0);
    ...
    return (
      <div className="space-y-8 animate-in fade-in duration-500">
        ...
        <div className="flex justify-between items-end border-b border-[#4e5254] pb-4">
          <div>
            <h1 className="text-3xl font-bold text-white mb-1">Project Portfolio</h1>
            <p className="text-[#808080] text-sm font-mono">Last synced: {new Date(stats.lastUpdated).toLocaleString()}</p>
          </div>
          <button 
            onClick={fetchStats}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
           <StatCard title="Total Solved" value={totalSolved} color="text-[#467CDA]" />
        ...
        {/* Platform Breakdown */}
        <h2 className="text-xl font-bold text-white mt-12 mb-4">Platform Modules</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
           <PlatformCard platform="LeetCode" data={stats.leetcode} themeColor="text-[#CC7832]" />
           <PlatformCard platform="Codeforces" data={stats.codeforces} themeColor="text-[#467CDA]" />
           <PlatformCard platform="GeeksForGeeks" data={stats.geeksforgeeks} themeColor="text-[#61C554]" />
        </div>
