...
const MergeSortVisualizer = () => {
    ...
    const resetArray = () => {
        if (isSorting) return;
        const newArr = Array.from({ length: 40 }, () => Math.floor(Math.random() * 400) + 20);
        setArray(newArr);
        setComparingIndices([]);
    ...
    const mergeSort = async () => {
        ...
        mergeSortHelper(arrCopy, 0, arrCopy.length - 1, auxiliaryArray, animationQueueRef.current);
    ...
    const doMerge = (mainArray, startIdx, middleIdx, endIdx, auxiliaryArray, animations) => {
        ...
        while (i <= middleIdx && j <= endIdx) {
            ...
            if (auxiliaryArray[i] <= auxiliaryArray[j]) {
                animations.push({ type: 'overwrite', idx: k, newHeight: auxiliaryArray[i] });
                mainArray[k++] = auxiliaryArray[i++];
            } else {
                animations.push({ type: 'overwrite', idx: k, newHeight: auxiliaryArray[j] });
        ...
        <div className="flex flex-col h-full bg-[#2B2B2B]">
           <div className="mb-6 border-b border-[#4e5254] pb-4 flex flex-col md:flex-row justify-between items-start md:items-center">
              <div>
                 ...
                 <p className="text-[#808080] font-mono text-sm">// Demonstrating classic Merge Sort Divide and Conquer</p>
              </div>
